...
namespace JA.Geometry
{
    public readonly struct Polygon 
    {
        ...
        public Polygon Transform(Matrix4x4 transform, bool inverse = false)
        {
            if (inverse)
            {
                Matrix4x4.Invert(transform, out var inv);
                return new Polygon(Nodes.Select((n) => Vector3.Transform(n, inv)).ToArray());
            }
            return new Polygon(Nodes.Select((n) => Vector3.Transform(n, transform)).ToArray());
        }
        public Polygon Rotate(Quaternion rotation)
        {
            return new Polygon(Nodes.Select((n) => Vector3.Transform(n, rotation)).ToArray());
        }
        public Polygon Rotate(Quaternion rotation, Vector3 pivot)
        {
            return new Polygon(Nodes.Select((n) => pivot + Vector3.Transform(n - pivot, rotation)).ToArray());
        }
